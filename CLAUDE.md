...
| [runtime-arp-transition.md](docs/runtime-arp-transition.md) | How the engine consumes a freshly-tightened ARP from the orchestrator without operator-driven restart.  Decision: option A (restart on σ_arp threshold crossing).  Triggers: σ_arp first-crosses pin threshold, mean ECEF drift > epsilon, mount_id increment.  What survives restart vs is rebuilt; what re-opens options B (hot-swap) / C (glide-slope).  I-145846. |
