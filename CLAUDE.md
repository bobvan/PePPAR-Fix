...
| [antenna-quality-metrics-and-timing.md](docs/antenna-quality-metrics-and-timing.md) | **Read before optimising an antenna for timing.**  Reconciles three code-minus-carrier metrics that differ 3x on the same antenna (testAnt `cmc_std` keeps the iono trend and is A/B-only; TEQC MP cancels iono exactly; onocoy's RMS Code is elevation-masked).  Then the part that matters: of onocoy's four RTK quality metrics, **phase RMS and cycle-slip rate predict timing, code RMS barely does** — code enters carrier-phase timing only via coarse sync and WL/MW fixing.  onocoy's 1.2 mm phase RMS IS our TD-CP noise (4.0 ps).  Also: relative field-calibration of an uncalibrated antenna via the UFO1/CHOKE1 swap, why PCV *shape* matters more than absolute PCO for a pinned-ARP timing receiver, and why a second co-located onocoy station is the wrong experiment. |
