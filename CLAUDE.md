...
| [arp-survey-strategy.md](docs/arp-survey-strategy.md) | **Forward plan for ARP acquisition (the load-bearing component, since position is pinned via survey).**  Tiered fallback to a ≤10 cm ARP: Tier 1 PRIDE PPP-AR (cm, fixed sites, product latency); Tier 2 NTRIP CORS-RTK (`--cors`, cm, fast, RTK-capable receivers like F9P — *the tidy-up*: promote the buried `--rtklib --cors-ntrip-*` path to a clean capability-gated backend); Tier 3 our own static-averaged RTKLIB-recipe PPP self-survey to ≤10 cm when neither products nor CORS exist (needs honest-σ first).  Receiver-capability matrix + selection logic.  Plan/design, not yet implemented. |
...
| [pcv-error-bounds-relative-to-pco.md](docs/pcv-error-bounds-relative-to-pco.md) | **Read before deploying an uncalibrated antenna with a borrowed PCO.**  Bounds the error of omitting/borrowing PCV when the PCO is estimated from a similar calibrated model.  ANTEX PCV is the residual relative to the PCO (mean ≈ 0 by construction; the **peak-to-peak** is the elevation-correlated uncorrected error, 1 mm ≈ 3.336 ps).  IGS igs20.atx: choke rings L1 p-p ~20 mm (old Dorne-Margolin 22–29, modern Leica/NovAtel 5–8), L2 ~15 mm; **SFESPK6618H (SparkFun lab antenna) is flatter than every choke ring at 3.3 mm L1 / 6.1 mm L2** — which justifies the zero-PCV APC survey choice for it (~11 ps) but NOT for a choke ring (~83 ps).  Tool: `tools/analyze_chokering_pcv.py` (`--tokens`/`--out-name`). |
