...
| [wl-integer-vs-pride-comparison.md](docs/wl-integer-vs-pride-comparison.md) | Engine WL-integer fixes vs PRIDE-PPP float WL on MadHat F9T DOY 126.  E10 wrong by 22 cycles (16.5 m bias contribution); E19 wrong by 55 cycles (41 m).  MW tracker's `int_history` shows cycle drift between successive fix attempts — likely cycle-slip-detector miss or PR-side noise inflating the running mean.  Smoking-gun source for I-155354's WL-only 6–8 m basin offset.  Comparison harness at scripts/replay/wl_vs_pride.py. |
