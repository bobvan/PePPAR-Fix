...
| [split-clock-architecture.md](docs/split-clock-architecture.md) | **Sketch + sanity test of the indoor/outdoor split clock** (rooftop "ear" = antenna + rx + PTP GM on PoE; indoor unit = holdover DO + multi-ear selection).  Precedent: this is a standardized class — ITU-T **T-BC-A** (G.8273.4/G.8275.2 APTS) is literally "a boundary clock assisted by a local time reference", and ePRTC (G.8272.1) already separates flywheel from GNSS; what's novel is the *direction* (GNSS remote, over PTP).  Findings: link asymmetry is 200-400 ns but **static and calibratable** via a one-time PPS coax at commissioning; **three ears on one roof are NOT three independent sources** (shared sky/firmware/corrections — the non-GNSS NTS/provider feeds are the real independence); **i226 is disqualified by our own igc wedge doc** (~44 min MTBF, no self-recovery, triggered by exactly this workload); multi-NIC = multi-PHC unless the DO disciplines all of them or you use one 4-port NIC (**E810-XXVDA4T, already in the lab**).  **Don't use BMCA to select upstream** — it ranks advertised attributes, not measurements; use Marzullo intersection over ears + non-GNSS feeds with the DO as flywheel, and make the indoor unit always-GM downstream so slaves never see a choice.  3f+1 ⇒ 3 ears + local holdover tolerates 1 Byzantine.  Mesh PTP (Timebeat **PTP+Squared**, **PTPsec** cyclic-path asymmetry) supplies the redundant measurements; the rejection logic is still yours.  Software forwarding is fine because **a BC does not forward PTP** — build a BC, never a TC. |
| [two-clock-agreement-forward-model.md](docs/two-clock-agreement-forward-model.md) | **The sync budget run forwards — parameters → p95, for sizing hardware before it's built.**  Same math as `two-site-sync-budget.md`, solved for the other variable.  Reframes the four budget terms as **three bands split by the loop time constant τ_c**: DO noise below the loop BW (grows with τ_c), observer noise the loop injects (shrinks as 1/√(f_s·τ_c)), and reference-side error the loop faithfully *tracks* (immune to tuning).  Bands 1+2 crossing sets the optimum τ_c.  Key results: EXTINT-only at 8 ns + hobby OCXO ⇒ **p95 ≈ 1 ns at τ_c ≈ 67 s** (three independent routes agree); above σ_ref ≈ 0.5 ns the TICC-vs-EXTINT choice stops mattering (explains that A/B's ambiguity); measured 8.04 ns back-fits to σ_ref ≈ 2.9 ns/clock.  **8 ns only averages down while the quantizer dithers** — clocking the receiver from the DO makes it a ±4 ns standing bias.  **PTP is not in this budget** (delivery leg, not agreement) — hence PPS IN/OUT on every unit.  Tool: `tools/sync_agreement_model.py` (+ tests). |
...
| [tdc-click-multichannel-ticc.md](docs/tdc-click-multichannel-ticc.md) | **Read before building any TICC alternative.**  Can MIKROE-4770 TDC Clicks make a multi-channel TICC equivalent?  Yes — the Click carries the same TDC7200 (identical ~55 ps resolution) and, crucially, has a `CLK SEL` EXT/INT jumper + coax so it can run on the house 10 MHz, plus separate START/STOP connectors.  But the TDC7200 is a *stopwatch*, not a timestamper: the TICC's real content is the 10 kHz coarse clock, the coarse counter, and the **stop gate** (blocks COARSE_CLOCK from STOP for 3 EXT_REF cycles after START, so the chip's 200 ns Mode-2 minimum is never violated) — none of which is on the Click.  Omitting the gate silently corrupts ~1 measurement in 500.  Host must keep a jitter-free 10 kHz coarse counter, so **not a Linux Pi**; recommends prototyping on a Click Shield for Pi Pico with RP2040 PIO doing divider+gate+counter in hardware.  TAPR App Note 2020-01 already documents 4 TICCs → 8 channels.  Payoff for us: whole-fleet PPS comparison on one timescale, not just a pair. |
