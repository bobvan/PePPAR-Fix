...
## Resolution (landed, 2026-05-10)

The engine-side per-filter MISS-bias gate (I-175645 Fix #2 / PR #20)
ships in the same PR as this survey doc.  After it lands, BDS B2a-I
observations are routed correctly even without phase-bias correction:

  - Dropped from PPPFilter / MW tracker / WL bootstrap (via
    `obs_for_position` helper at `scripts/peppar_fix/obs_routing.py`)
    so the half-corrected MW combination doesn't basin position.
  - Kept for FixedPosFilter via TD-CP — the phase-bias constant
    cancels epoch-to-epoch (`delta_phi = phi_if_m[t] − phi_if_m[t−1]`
    at `solve_ppp.py:1389-1393`), so BDS contributes ~6–8 SVs/epoch
    to time-side `dt_rx` σ tightening without poisoning position.

Two affected hosts have BDS in their `systems=` today:
...
These STAY at `gps,gal,bds` post-Fix #2 — Bob's framing (2026-05-10
morning, before this PR landed) was: "even without phase biases,
BDS could still be useful for time."  TD-CP cancellation makes that
true; the engine-side gate ensures the position filter doesn't pay
the cost.
...
4-fix combined test on MadHat (PR #17 + bravo's PR #19 fix-I + fix-H
+ this PR's Fix #2) at `test-merge-bravo-charlie @ c5b90ee`, 29 min
uptime, `--ar-mode wl --systems gps,gal,bds`:

  - AntPosEst ZTD residual: −1770..+6205 mm → **+1 mm** (within
    60 mm of PRIDE same-antenna truth −61 mm)
  - σ_dt_rx: 0.16 → **0.10 ns** (more time-side SVs)
  - 349 BDS B2a-I MISS events properly routed — dropped from
    PPPFilter, retained in FixedPosFilter via TD-CP
  - AntPosEst basin offset: 2.5 m horizontal / 0.31 m vertical at
    29 min, monotone tightening

H1 (BDS MISS still emits) + H2 (basin shrinks) + H3 (`σ_dt_rx`
tightens) all confirmed.
