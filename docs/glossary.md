...
| **integer fix** | A per-SV integer assignment of a carrier-phase ambiguity (WL, NL, or both).  Distinct from a *position fix*; we never overload "fix" for both — position gets "solution".  See SvAmbState. |
| **self-consistent integer set** | A set of per-SV integer ambiguities that, taken as a group, are mutually consistent with the observed dual-frequency phase data.  LAMBDA's job is to **identify** a candidate self-consistent set from the float ambiguity covariance (a joint-MAP pick, not per-SV independent picks).  The validation promoter's job is to **prove** the set stays self-consistent as satellite geometry evolves (≥ 15° of Δaz).  Long-term members form the set demonstrated self-consistent temporally and geometrically; short-term members were consistent at their promotion moment but are on probation.  The false-fix monitor detects breakdown of self-consistency — a set that looked good at fix time but doesn't survive geometry change. |
