...
| **CNES** | Centre National d'Etudes Spatiales (French space agency).  Provides single-AC SSR corrections on `products.igs-ip.net` mount `SSRA00CNE0`.  CNES phase biases match F9T Galileo signals (L1C, L5Q, L7Q).  Used as the production orbit/clock source. |
| **WHU** | Wuhan University.  OSB (observation-specific bias) stream on `products.igs-ip.net` mount `OSBC00WHU1`.  **Biases-only** — pair with CNES (or another AC) for orbit/clock.  Best phase-bias datum match for F9T at our site (2026-04-25 2x2 isolation: WHU biases land 1.59 m east of CNES biases on same orbit/clock, closer to truth). |
| **CAS** | Chinese Academy of Sciences.  SSR provider on `SSRA01CAS1` via `ntrip.data.gnss.ga.gov.au`.  Uses **IGS-SSR `4076_*` proprietary message IDs** instead of standard RTCM 1057-1068/1240-1263.  Originally produced 280 m position divergence — root cause was an engine bug treating pyrtcm's mm-output IDF013-021 as metres (1000× too large).  **Fixed 2026-04-25 commit 485612d.**  Post-fix CAS produces stable PPP solutions in the same regime as CNES (~m offset from truth, sub-meter cohort).  Multi-constellation `gps,gal,bds` works on CAS post-fix. |
...
| **OU process** | Ornstein-Uhlenbeck process.  A mean-reverting stochastic process governed by `dx = -θ(x−μ)dt + σ dW` — combines a random-walk-like diffusion with a restoring force toward long-run mean `μ` at rate `θ`.  Contrasts with a pure random walk (`dx = σ dW`), which has no restoring force and wanders unboundedly.  Relevant to PePPAR Fix because real atmospheric ZTD *is* mean-reverting (wet-delay fluctuates around a slow seasonal/weather-driven average, it doesn't drift to ±∞) — modelling ZTD as a random walk in the filter's process model therefore misrepresents the physics and lets the filter absorb null-mode drift into ZTD without the natural physical resistance.  Switching the ZTD process model from random-walk to OU is one of the candidate "structural" (not tuning) fixes for the null-mode floor identified by the 2026-04-23 PRIDE tuning sweep.  See `project_to_main_filter_tuning_result_20260423.md`. |
...
| **PAR-ready** | The float ambiguity covariance is precise enough that LAMBDA's *partial* AR — fixing some ambiguities while leaving the worst as floats — is likely to succeed, but the full set is not yet ready.  Operationally: bootstrap success rate `P_IB ≥ 0.99` and `< 0.999`.  Above 0.999 = "full AR green-light"; below 0.99 = "diagnose Q_â per-SV".  Threshold convention from Geng et al. 2010.  Computed independently for WL and NL.  Surfaced as `[WL_AR_READINESS]` / `[AR_READINESS]` engine log lines and as the `WL P_IB` / `NL P_IB` peppar-mon widget — green for ≥ 0.999 (full), yellow for ≥ 0.99 (PAR-ready), red below.  See `ppp_ar.py:280` (`p_wl_ib` computation) and `peppar_mon/widgets.py` (display). |
