...
(`do_freq_est.py:567-569`) and in the routedQErrArm router
(`do_freq_est.py:471`, `:728`; tests in
`test_routed_qerr_arm.py`, `test_routed_qerr_observability.py`).

**Claim**: the name `_qerr` and the route label `internal-qerr`
both imply "quantization error" — the sub-nanosecond residual
between the receiver's 1 PPS edge and GPS time, traditionally
delivered by UBX-TIM-TP's `qErr` field at ~3 ns precision.

**Actual**: `_qerr(x[0])` is the *filter's predicted* PPS-edge
phase given the rx-TCXO state x[0].  It's a model output, not a
firmware measurement.  Its derivation is the carrier-phase
PPP-`dt_rx` to PPS-edge mapping that
`docs/pps-ppp-error-source.md` describes — a phase offset between
GPS time and the rx TCXO, not a quantization residual.

**Why it matters**: confuses the failure-mode analysis on
receivers that don't emit usable TIM-TP qErr (F9P, X20P HPG with
`qErrInvalid=1`).  Operators reading the routedQErrArm router
output reasonably ask "if external qErr is dead, does the
internal qerr replace it?" — the answer is *no, it's a different
quantity entirely*, but the naming hides that.  The router falling
through `external → internal-qerr → raw` reads as "use the
firmware estimate, else use our own estimate, else give up,"
when in reality the three branches are different observables
with different dimensional meaning.  Surfaced 2026-06-15 while
investigating PiFace overnight cascade (`pifaceOvernightNoLock`,
I-100945-main): a starved x[0] makes `_qerr(x[0])` mis-predict,
Arm 4's "corrected" differential gets chi²-rejected, the
