...
# Absolute σ fallback gate when NAV2 hAcc is unavailable (e.g. NAV2
# fix lost transiently).  Writing this is "worse than NAV2's typical
# hAcc on a clean-sky lab host" so we cap the snapshot quality at
# this level.  Used only when the preferred relative gate
# (PPP_WRITE_NAV2_BETTER_RATIO * nav2_h_acc_m) can't be evaluated.
DEFAULT_PPP_WRITE_MAX_SIGMA_M = 1.0

# Preferred gate: PPP solution σ must be at least this much smaller
# than NAV2's reported hAcc to be worth saving.  Default 0.5 means
# "2× better than NAV2" — the snapshot is only saved when the PPP
# solution would actually beat the NAV2 cold-start fallback by a
# meaningful margin.  Filters out early-epoch PPP estimates whose σ
# happens to be < 1 m but which still aren't tighter than NAV2's
# live fix.  Motivated by the 2026-05-18 TimeHat incident where an
# n_epochs=1 PPP snapshot with σ=0.95 m was promoted to "survey"
# despite NAV2 reporting hAcc=0.78 m (PPP wasn't actually better).
DEFAULT_PPP_WRITE_NAV2_BETTER_RATIO = 0.5
...
class PppStateWriter:
    ...
    ``maybe_write(pos_ecef, sigma_3d, n_epochs, nav2_h_acc_m=...)``
    every AntPosEst epoch; this class decides whether to actually
    persist based on:

      - ``receiver_uid`` not None (else no-op for tests/dev)
      - quality gate: prefer ``sigma_3d ≤ nav2_better_ratio *
        nav2_h_acc_m`` (PPP solution must beat NAV2 by the configured
        margin); fall back to ``sigma_3d ≤ max_sigma_m`` (absolute)
        only when ``nav2_h_acc_m`` is None or non-positive
    ...
    def __init__(self,
                 ...
                 *,
                 ...
                 nav2_better_ratio: float
                     = DEFAULT_PPP_WRITE_NAV2_BETTER_RATIO,
        ...
        self.nav2_better_ratio = float(nav2_better_ratio)
    ...
    def maybe_write(self, pos_ecef, sigma_3d: float,
                    n_epochs: int,
                    nav2_h_acc_m: Optional[float] = None) -> bool:
        """Write a PositionState snapshot if the gates allow.

        Returns True iff a write actually happened.

        Quality gate prefers ``sigma_3d ≤ nav2_better_ratio *
        nav2_h_acc_m`` (relative, depends on current NAV2 quality);
        falls back to ``sigma_3d ≤ max_sigma_m`` (absolute) only when
        ``nav2_h_acc_m`` is None or non-positive — covers the case
        where NAV2 fix is currently lost but AntPosEst is still
        producing a meaningful estimate.
        """
        if self.receiver_uid is None:
            return False
        # Quality gate.  Prefer "twice as tight as NAV2"; fall back to
        # absolute when NAV2 hAcc unavailable.
        if nav2_h_acc_m is not None and nav2_h_acc_m > 0:
            threshold = self.nav2_better_ratio * nav2_h_acc_m
            gate_basis = (f"nav2_h_acc_m={nav2_h_acc_m:.3f}m × "
                          f"ratio={self.nav2_better_ratio}")
        else:
            threshold = self.max_sigma_m
            gate_basis = (f"max_sigma_m={self.max_sigma_m:.3f}m "
                          "(NAV2 hAcc unavailable)")
        if sigma_3d > threshold:
            self.n_skipped_sigma += 1
            return False
        now = self._time_fn()
        ...
        self._next_write_t = now + self.period_s
        log.info(
            "[PPP_STATE_WRITE] σ=%.4fm gate=%s n_epochs=%d "
            "mount_sn=%d → state/positions/%s.ppp.toml",
            sigma_3d, gate_basis, n_epochs, self.mount_sn,
            self.receiver_uid,
        )
        return True
